...
def persist_text_inbox(chat_id: int, msg_id: int, body: str, sender: str) -> str:
...
def persist_media_inbox(chat_id: int, msg_id: int, kind: str, filename: str, sender: str, caption: str = "") -> str:
...
def process_message(msg: dict):
    ...
    def _ack(kind: str, filename: str):
        ...
        if kind != "text" and (not is_private_chat) and is_full_chat_observed(chat_id):
            media_slug = persist_media_inbox(chat_id, int(msg_id), kind, filename, sender_label(msg), caption)
            if media_slug:
    ...
    if "text" in msg:
        ...
        if (not is_private_chat) and is_full_chat_observed(chat_id):
            observed_slug = persist_text_inbox(chat_id, int(msg_id), body, sender_label(msg))
            if observed_slug:
        ...
        if (not is_private_chat) and body.strip() and not command_body.startswith("/"):
            ...
            _slug = persist_text_inbox(chat_id, int(msg_id), body, "madi")
            if _slug:
