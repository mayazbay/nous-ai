...
def build_corpus(*subdirs, exclude_paths=None):
    """Concatenated lowercase markdown content under given subdirs.

    `exclude_paths` is a set of relative-to-ROOT paths to skip — used to remove
    the reachability audit receipt itself from the corpus, since listing
    orphans in the audit shouldn't auto-validate them.
    """
...
def main():
    ...
    skills_laws_corpus = build_corpus(
        "pages/skills",
        "pages/laws",
        "pages/audits",
        "pages/specs",
    ...
    for p in audit_pages:
        ...
        in_prose = in_wl or has_word_match(skills_laws_corpus, b)
