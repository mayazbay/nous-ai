...
def status_from_satory_queue(payload: Any) -> tuple[str, str]:
    if not isinstance(payload, dict):
        return "RED", "Satory queue dry-run did not return JSON"
    selected = int(payload.get("selected") or 0)
    if selected > 0:
        return "GREEN", f"selected={selected}; dry-run ok"
    diagnostics = payload.get("diagnostics")
    if isinstance(diagnostics, dict):
        unblocked = int(diagnostics.get("unblocked_candidates") or 0)
        ledger_blocked = int(diagnostics.get("ledger_blocked_candidates") or 0)
        queued = int(diagnostics.get("queued_candidates") or 0)
        if unblocked == 0 and ledger_blocked > 0:
            return "GREEN", (
                "selected=0; all eligible queued candidates are ledger-covered "
                f"(queued={queued} ledger_blocked={ledger_blocked})"
            )
        if unblocked == 0:
            return "GREEN", f"selected=0; no unblocked queued candidates (queued={queued})"
        return "YELLOW", f"selected=0 but {unblocked} unblocked candidate(s) were not selected"
    return "YELLOW", "selected=0; dry-run lacks queue diagnostics"
...
def status_from_gbrain_doctor(output: str, returncode: int) -> tuple[str, str, Any | None]:
    ...
    if status in {"ok", "healthy"}:
        return "GREEN", f"status={status} health_score={score}", payload
    if status == "warnings":
...
def latest_handoff_path() -> str:
    result = ssh(
        VPS_HOST,
        "ls -t /root/nous-agaas/wiki/pages/progress/HANDOFF-AUTO-*.md 2>/dev/null | head -1",
        timeout=30,
    )
    path = result.stdout.strip()
    marker = "/wiki/"
    if result.ok and marker in path:
        return path.split(marker, 1)[1]
    return "pages/progress/HANDOFF-AUTO-2026-05-20-09-00.md"
...
def check_openclaw_current_runtime_health() -> dict[str, Any]:
    result = ssh(
        "air",
        (
            "printf 'health='; curl -fsS --max-time 5 http://127.0.0.1:18789/health; "
            "printf '\\ninspect='; "
            "docker inspect openclaw --format '{{.Config.Image}}\t{{.State.Status}}\t{{if .State.Health}}{{.State.Health.Status}}{{end}}' 2>/dev/null; "
            "printf '\\nlegacy18790='; curl -fsS --max-time 2 http://127.0.0.1:18790/health 2>&1 || true"
        ),
        timeout=30,
    )
    text = (result.stdout + result.stderr).strip()
    prod_live = result.ok and 'health={"ok":true,"status":"live"}' in text
    prod_current = "ghcr.io/openclaw/openclaw:2026.5.19\trunning\thealthy" in text
    if prod_live and prod_current:
        return check(
            "openclaw_current_runtime_health",
            "GREEN",
            "production OpenClaw 2026.5.19 is healthy on 18789; stale 5.18 sidecar is not required",
            text[-1000:],
        )
    if prod_live:
        return check(
            "openclaw_current_runtime_health",
            "GREEN",
            "production OpenClaw is live on 18789",
            text[-1000:],
        )
    return check(
        "openclaw_current_runtime_health",
        "YELLOW",
        "production OpenClaw 18789 runtime not fully proven",
        text[-500:],
    )


def check_control_plane_sync() -> dict[str, Any]:
    result = ssh(
        "air",
        "cd ~/nous-agaas/wiki && python3 tools/control_plane_sync_loop.py --dry-run --json --no-telegram --no-apply-todoist",
        timeout=300,
    )
    payload = json_from(result)
    status, detail = status_from_control_plane(payload)
    return check(
        "control_plane_sync_dry_run",
        status,
        f"Air {detail}",
        payload if payload is not None else summarize_command_failure(result),
    )
...
def check_satory_queue(wiki: Path) -> dict[str, Any]:
    ...
    status, detail = status_from_satory_queue(payload)
    evidence = {
        "selected": payload.get("selected"),
        "results": payload.get("results", [])[:3],
        "diagnostics": payload.get("diagnostics"),
    }
    return check("satory_queue_dry_run", status, detail, evidence)
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    wiki = args.wiki
    latest_handoff = args.latest_handoff or latest_handoff_path()
    ...
    checks.append(check_control_plane_sync())
    ...
    checks.append(check_qmd_exact_handoff(latest_handoff))
    checks.append(check_satory_queue(wiki))
    checks.append(check_openclaw_current_runtime_health())
    ...
    result["latest_handoff"] = latest_handoff
