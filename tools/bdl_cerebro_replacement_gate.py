...
def _raw_intake_check(source: QuerySource, now: dt.datetime, fresh_hours: float) -> dict[str, Any]:
    ...
    # v1.4.0 (2026-05-25): per-row received_at freshness filter on parsed/quarantined SUMs.
    # Without this, a historical 9000-row backfill of parsed=1 + one fresh row would
    # report parsed=9001 + MAX=fresh and pass GREEN even if no parsing is happening now.
    # See AP-7 (extended) in bdl-cerebro-replacement-gate v1.4.0.
    cutoff = (now - dt.timedelta(hours=fresh_hours)).isoformat()
    row = _row(source.sql(
        "events",
        "SELECT COUNT(*), MAX(received_at), "
        f"SUM(CASE WHEN parse_status='parsed' AND received_at >= '{cutoff}' THEN 1 ELSE 0 END), "
        f"SUM(CASE WHEN parse_status='quarantined' AND received_at >= '{cutoff}' THEN 1 ELSE 0 END), "
        f"SUM(CASE WHEN received_at >= '{cutoff}' THEN 1 ELSE 0 END) FROM raw_events;",
    ))
    total = _int(row, 0)
    latest = _parse_dt(_get(row, 1))
    parsed_fresh = _int(row, 2)
    quarantined_fresh = _int(row, 3)
    fresh_total = _int(row, 4)
    age_h = _age_hours(now, latest)
    if total <= 0:
        return {"check": "raw_intake", "status": YELLOW, "detail": "raw_events has no intake records"}
    if age_h <= fresh_hours and parsed_fresh > 0:
        status = GREEN
    elif age_h <= fresh_hours and fresh_total > 0 and quarantined_fresh >= fresh_total:
        status = YELLOW
    else:
        status = YELLOW
    return {
        ...
        "status": status,
        "detail": (
            f"raw_events total={total} fresh_total={fresh_total} parsed_fresh={parsed_fresh} "
            f"quarantined_fresh={quarantined_fresh} latest={_iso(latest)} age_h={_round(age_h)}"
        ),
    }


def _fleet_health_check(source: QuerySource, now: dt.datetime, health_fresh_hours: float) -> dict[str, Any]:
    # v1.3.0 (2026-05-25): per-row last_check freshness filter on the online
    # count. Prior behaviour summed status='online' across ALL rows including
    # rows whose last_check was 55+ days stale (AUDIT-fleet-zero-online-
    # 20day-silent-outage-2026-05-25). A subnet drop from probe iteration
    # left 38 rows frozen as online; the gate read them as still-online and
    # reported "13.5% online" while reality was 0%. Filter applies the same
    # health_fresh_hours window as the gate-level age check.
    cutoff = (now - dt.timedelta(hours=health_fresh_hours)).isoformat()
    row = _row(source.sql(
        "health",
        "SELECT COUNT(*), "
        f"SUM(CASE WHEN status='online' AND last_check >= '{cutoff}' THEN 1 ELSE 0 END), "
        f"SUM(CASE WHEN status='online' AND last_check < '{cutoff}' THEN 1 ELSE 0 END), "
        "MAX(last_check) FROM camera_status;",
    ))
    total = _int(row, 0)
    online_fresh = _int(row, 1)
    stale_online = _int(row, 2)
    latest = _parse_dt(_get(row, 3))
    age_h = _age_hours(now, latest)
    online_pct = (online_fresh / total) if total else 0.0
    detail = f"camera_status total={total} online_fresh={online_fresh} stale_online={stale_online} online_pct={online_pct:.3f} latest_check={_iso(latest)} age_h={_round(age_h)}"
    if total <= 0:
    ...
    if online_fresh <= 0:
        return {"check": "fleet_health", "status": RED, "detail": f"{detail}; ZERO cameras online with current last_check"}
    if online_pct < 0.85:
...
def _law002_check(source: QuerySource, now: dt.datetime, fresh_hours: float) -> dict[str, Any]:
    # v1.4.0 (2026-05-25): per-row event_time freshness filter for count_fresh.
    # Without this, an old backfill of 1000 historical violations + one new event
    # would report count=1001 + MAX=fresh and pass GREEN even if classification
    # has actually stopped happening. See AP-7 (extended) in bdl-cerebro v1.4.0.
    cutoff = (now - dt.timedelta(hours=fresh_hours)).isoformat()
    row = _row(source.sql(
        "events",
        "SELECT COUNT(*), MAX(event_time), "
        f"SUM(CASE WHEN event_time >= '{cutoff}' THEN 1 ELSE 0 END) FROM vehicle_events "
        "WHERE (vehicle_speed - speed_limit) >= 10 "
        "AND plate_confidence >= 80 "
        "AND is_violation = 1;",
    ))
    count_total = _int(row, 0)
    latest = _parse_dt(_get(row, 1))
    count_fresh = _int(row, 2)
    age_h = _age_hours(now, latest)
    detail = f"LAW-002 classified violations count_total={count_total} count_fresh={count_fresh} latest={_iso(latest)} age_h={_round(age_h)}"
    if count_total <= 0:
        return {"check": "law002_classification", "status": RED, "detail": "no LAW-002 compliant classified violations found"}
    if count_fresh <= 0:
        return {"check": "law002_classification", "status": RED, "detail": f"{detail}; ZERO fresh classified violations within {fresh_hours}h window"}
    if age_h <= fresh_hours:
...
def _erap_queue_check(source: QuerySource, now: dt.datetime, fresh_hours: float) -> dict[str, Any]:
    ...
    # v1.4.0 (2026-05-25): per-row updated_at freshness filter on per-status counts.
    # Without this, a queue with 3 stale 'pending' rows from 68 days ago + 0 fresh
    # would report `states={'pending': {'count': 3, ...}}` and operators couldn't
    # tell if the queue is actively moving or just stuck. See AP-7 (extended).
    cutoff = (now - dt.timedelta(hours=fresh_hours)).isoformat()
    rows = _rows(source.sql(
        "queue",
        "SELECT status, COUNT(*), MAX(updated_at), "
        f"SUM(CASE WHEN updated_at >= '{cutoff}' THEN 1 ELSE 0 END) FROM submission_queue "
        "GROUP BY status ORDER BY status;",
    ))
    if not rows:
        return {"check": "erap_queue", "status": YELLOW, "detail": "submission_queue exists but is empty"}
    states = {r[0]: {"count_total": _int(r, 1), "latest": _get(r, 2), "count_fresh": _int(r, 3)} for r in rows}
    latest = max((_parse_dt(v["latest"]) for v in states.values()), default=None)
    age_h = _age_hours(now, latest)
    fresh_total = sum(v["count_fresh"] for v in states.values())
    ...
    if unknown:
        ...
        status = YELLOW
        detail_suffix = f"; queue visible but ZERO fresh activity within {fresh_hours}h (all rows stale)"
    ...
    return {
        "check": "erap_queue",
        "status": status,
        "detail": f"submission_queue states={states} latest_age_h={_round(age_h)} fresh_total={fresh_total}{detail_suffix}",
        "states": states,
    }
