...
TS_IP="100.70.222.21"
GPU_REQUIRED=0
case "${NOUS_GPU_REQUIRED:-}" in
  1|true|TRUE|yes|YES) GPU_REQUIRED=1 ;;
esac
...
degrade_or_fail() {
  ...
  log "SKIP optional Nous-GPU collector degraded — $1 (set NOUS_GPU_REQUIRED=1 for GPU-bound runs)"
  echo 0 > "$STATE_LAST"
  exit 0
}
...
if ! ping -c 1 -W 5 "$TS_IP" >/dev/null 2>&1; then
  if ! ssh -o BatchMode=yes -o ConnectTimeout=5 -o StrictHostKeyChecking=no \
    ...
    degrade_or_fail "Tailscale unreachable $TS_IP (ICMP >5s AND SSH unreachable)"
  fi
...
if ! ssh -o BatchMode=yes -o ConnectTimeout=5 nous-gpu \
  ...
  degrade_or_fail "docker container '$NAME' not running on nous-gpu"
fi
...
if [ -z "$LATEST_PCAP" ]; then
  degrade_or_fail "no pcap written in last ${MTIME_WINDOW_MIN} min under $PCAP_DIR — $(wg_context)"
fi
...
if [ "$delta" -lt "$MIN_DELTA_BYTES" ]; then
  degrade_or_fail "pcap ${LATEST_PCAP##*/} delta=+$delta B (< $MIN_DELTA_BYTES min) — $(wg_context)"
fi
