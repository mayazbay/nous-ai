...
# ---------------------------------------------------------------------------
# v1.5.0 — Council adapters for weekly_model_council.py (moonlit-pnueli P3.3)
# Generic LiteLLM-routed helpers so any model wired into Air:4000 LiteLLM is
# callable from the council. All paid-API-gated via _paid_api_block_reason.
# ---------------------------------------------------------------------------

# Model IDs (matched against Air LiteLLM config aliases — verified 2026-05-23)
# Available in Air's LiteLLM today: opus, sonnet, sonnet-4-5-thinking, haiku-4-5,
# glm-5.1, glm-4.5-flash, deepseek-v4-flash, deepseek-v4-pro, grok-reasoning,
# grok-code-fast, kimi-k2.6, local-mlx-coder, text-embedding-3-large.
# Gemini / Qwen / Composer are NOT yet wired into Air's LiteLLM and would 404.
# They will be added once LiteLLM config is updated (separate change).
MODEL_DEEPSEEK_PRO = "deepseek-v4-pro"  # ✅ wired
MODEL_KIMI = "kimi-k2.6"                # ✅ wired (Moonshot K2 v2.6)
MODEL_GEMINI = "gemini-2-5-pro"         # ❌ not yet in Air LiteLLM — adapter will 404
MODEL_QWEN = "qwen3-coder"              # ❌ not yet in Air LiteLLM — adapter will 404
MODEL_COMPOSER = "composer-1"           # ❌ not yet in Air LiteLLM — adapter will 404

# Approximate per-million-token pricing (OpenRouter posted, May 2026)
COUNCIL_PRICING: dict[str, tuple[float, float]] = {
    MODEL_DEEPSEEK_PRO: (0.55, 2.19),   # post-promo; pre-2026-05-31 is ~25% of this
    MODEL_KIMI: (0.60, 2.50),
    MODEL_GEMINI: (1.25, 5.00),
    MODEL_QWEN: (0.40, 1.20),
    MODEL_COMPOSER: (1.00, 3.00),       # placeholder until Cursor publishes API rates
}

# Default council = all 8 adapters now that gemini/qwen/composer landed in Air's
# LiteLLM (config.yaml updated 2026-05-23, see RESOLVER.md xai-premium-tools row
# and pages/audits/RECEIPT-litellm-config-add-3-aliases-2026-05-23.md).
# Sat-council launchd plist invokes the driver without --models.
# Known per-model issues (Codex follow-up, NOT blocking):
#   - kimi-k2.6 and gemini-2-5-pro are reasoning models — may emit
#     finish_reason=length with completion_tokens consumed by reasoning_tokens.
#     The defensive _call_via_litellm raises empty_or_null_content cleanly so
#     the council degrades to YELLOW rather than crashing.
#   - codex-gpt-5.5 returns empty subscription stdout in subprocess context
#     (separate Codex CLI session/quota investigation needed).
DEFAULT_COUNCIL_KEYS = ("opus", "codex", "grok", "gemini", "deepseek_pro", "kimi", "qwen", "composer")


# Reasoning models consume max_tokens budget on reasoning_tokens FIRST and
# return empty completion_tokens with finish_reason=length if budget too tight.
# Per Stage 4→7→8 council validation evidence: kimi-k2.6 + gemini-2-5-pro +
# deepseek-v4-pro all hit this quirk. Gemini was fixed at 4000; Kimi STILL
# consumed all 4000 on reasoning → bump to 8000. DeepSeek-Pro added to set
# after Stage 8 council re-run showed it's also a reasoning model.
REASONING_MODELS = {MODEL_KIMI, MODEL_GEMINI, MODEL_DEEPSEEK_PRO}
...
def _call_via_litellm(
    ...
    context: str,
    model_id: str,
    billing_surface: str = BILLING_OPENROUTER,
    confidence: float = 0.80,
    timeout_s: int = MODEL_TIMEOUT_S,
) -> dict[str, Any]:
    """Generic LiteLLM (Air:4000) caller for any OpenRouter-routed council model.

    Cost is computed from COUNCIL_PRICING if the model is known, otherwise
    reported as 0.0 with a `cost_unknown: true` marker. Reasoning models
    (per REASONING_MODELS set) get a higher max_tokens budget so they don't
    burn the entire budget on internal reasoning_tokens with zero completion.
    """
    t0 = time.monotonic()
    # Per-model timeout bump for reasoning models (LiteLLM config gives them
    # 300s server-side; we mirror here so the HTTP client doesn't bail early).
    effective_timeout = max(timeout_s, 240) if model_id in REASONING_MODELS else timeout_s
    effective_max_tokens = REASONING_MAX_TOKENS if model_id in REASONING_MODELS else DEFAULT_MAX_TOKENS
    try:
        block_reason = _paid_api_block_reason(billing_surface, model_id)
        if block_reason:
            raise PermissionError(block_reason)
        ...
        resp = _http_post_json(LITELLM_BASE_URL, payload, headers, timeout=effective_timeout)
        answer = resp.get("choices", [{}])[0].get("message", {}).get("content")
        if not answer or not isinstance(answer, str) or not answer.strip():
            raise RuntimeError(f"empty_or_null_content: usage={resp.get('usage')} finish_reason={resp.get('choices', [{}])[0].get('finish_reason')}")
        usage = resp.get("usage", {})
        in_tok = usage.get("prompt_tokens", 0)
        out_tok = usage.get("completion_tokens", 0)
        prices = COUNCIL_PRICING.get(model_id)
        if prices:
            in_p, out_p = prices
...
def _call_gemini(question: str, context: str) -> dict[str, Any]:
    return _call_via_litellm(question, context, MODEL_GEMINI, confidence=0.85)


def _call_deepseek_pro(question: str, context: str) -> dict[str, Any]:
    return _call_via_litellm(question, context, MODEL_DEEPSEEK_PRO, confidence=0.80)


def _call_kimi(question: str, context: str) -> dict[str, Any]:
    return _call_via_litellm(question, context, MODEL_KIMI, confidence=0.75)


def _call_qwen(question: str, context: str) -> dict[str, Any]:
    return _call_via_litellm(question, context, MODEL_QWEN, confidence=0.75)


def _call_composer(question: str, context: str) -> dict[str, Any]:
    return _call_via_litellm(question, context, MODEL_COMPOSER, confidence=0.75)


COUNCIL_ADAPTERS: dict[str, tuple[Any, str]] = {
    "opus": (_call_opus, MODEL_OPUS),
    "codex": (_call_codex, MODEL_CODEX),
    "grok": (_call_grok, MODEL_GROK),
    "gemini": (_call_gemini, MODEL_GEMINI),
    "deepseek_pro": (_call_deepseek_pro, MODEL_DEEPSEEK_PRO),
    "kimi": (_call_kimi, MODEL_KIMI),
    "qwen": (_call_qwen, MODEL_QWEN),
    "composer": (_call_composer, MODEL_COMPOSER),
}
...
def council_run(
    ...
    cap_usd: float = 3.00,
    dry_run: bool = False,
    wallclock_cap_s: int = 540,
) -> dict[str, Any]:
    """Parallel council call across N council adapters.

    `models` is a list of keys from COUNCIL_ADAPTERS. Defaults to all 8.

    Returns a dict with `answers` (list of per-model dicts), `total_cost_usd`,
    `ok_count` (4-of-8 floor used by caller for YELLOW vs GREEN), and `cost_capped`
    if the total exceeded cap_usd.
    """
    models = models or list(COUNCIL_ADAPTERS.keys())
    unknown = [m for m in models if m not in COUNCIL_ADAPTERS]
    if unknown:
        raise ValueError(f"unknown council adapter keys: {unknown}")
    ...
    t0 = time.monotonic()
    answers: list[dict[str, Any]] = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=max(len(models), 1)) as executor:
        future_to_key = {}
        for key in models:
            adapter, _model_id = COUNCIL_ADAPTERS[key]
            # _call_opus accepts an extra local_answer arg (default None)
            future_to_key[executor.submit(adapter, question, context)] = key
        for fut in concurrent.futures.as_completed(future_to_key, timeout=wallclock_cap_s):
            key = future_to_key[fut]
            try:
                answers.append(fut.result())
            except Exception as exc:
                _adapter, model_id = COUNCIL_ADAPTERS[key]
                answers.append({
                    "model": model_id,
                    "key": key,
                    "error": f"{type(exc).__name__}: {exc}",
    ...
    ok_count = sum(1 for a in answers if "answer" in a)
    cost_capped = total_cost > cap_usd
