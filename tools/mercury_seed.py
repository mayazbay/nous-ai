...
def fact_identity(fact_obj: dict) -> tuple[str, str, str]:
    """Stable identity for preserving mutable metadata across regenerations."""
    return (
        str(fact_obj.get("subject", "")),
        str(fact_obj.get("value_hash", "")),
        str(fact_obj.get("source", "")),
    )


def load_existing_facts(path: Path = OUT) -> dict[tuple[str, str, str], dict]:
    if not path.exists():
        return {}
        ...
        with path.open(encoding="utf-8") as f:
            for line in f:
                ...
                if isinstance(obj, dict):
                    existing[fact_identity(obj)] = obj
    except (OSError, json.JSONDecodeError):
        return {}
    return existing


def preserve_existing_metadata(facts: list[dict], existing: dict[tuple[str, str, str], dict]) -> None:
    """Avoid dirty churn by preserving metadata when the underlying fact is unchanged.

    Mercury refresh runs from launchd. If unchanged permanent facts receive
    today's date every cycle, the tracked facts file becomes dirty even though
    no source doctrine changed.
    """
    preserve_keys = (
        "id",
        "freshness",
    ...
    for fact_obj in facts:
        old = existing.get(fact_identity(fact_obj))
        if not old:
            continue
        for key in preserve_keys:
            if key in old:
                fact_obj[key] = old[key]


def _fact_id_number(fid: str) -> int | None:
    match = re.fullmatch(r"fact-(\d+)", str(fid or ""))
    return int(match.group(1)) if match else None


def assign_ids_for_new_facts(
    facts: list[dict],
    existing: dict[tuple[str, str, str], dict] | None = None,
) -> None:
    existing = existing or {}
    existing_ids = {
        str(f.get("id"))
        for f in existing.values()
        if _fact_id_number(str(f.get("id"))) is not None
    }
    generated_ids = {
        str(f.get("id"))
        for f in facts
    ...
    reserved = existing_ids | generated_ids
    max_num = max((_fact_id_number(fid) or 0 for fid in reserved), default=0)
    next_num = max_num + 1
    seen: set[str] = set()
    ...
    for idx, fact_obj in enumerate(facts):
        ...
        if _fact_id_number(old_id) is None or old_id in seen:
            continue
        fact_obj["id"] = old_id
        seen.add(old_id)
        preserved_indexes.add(idx)

    for idx, fact_obj in enumerate(facts):
        if idx in preserved_indexes:
            continue
        fid = str(fact_obj.get("id", ""))
        if (
            _fact_id_number(fid) is not None
            and fid not in seen
            and fid not in existing_ids
        ):
            seen.add(fid)
            continue
        while True:
            candidate = f"fact-{next_num:05d}"
            next_num += 1
            if candidate not in reserved and candidate not in seen:
                fact_obj["id"] = candidate
                seen.add(candidate)
                reserved.add(candidate)
                break


def render_jsonl(facts: list[dict]) -> str:
    return "".join(json.dumps(fact_obj, ensure_ascii=False) + "\n" for fact_obj in facts)
...
def main():
    ...
    existing_facts = load_existing_facts(OUT)
    preserve_existing_metadata(facts, existing_facts)
    assign_ids_for_new_facts(facts, existing_facts)
    ...
    changed = write_if_changed(OUT, render_jsonl(facts))
    size_kb = OUT.stat().st_size / 1024
    verb = "wrote" if changed else "unchanged"
    print(f"\n✅ {verb} {len(facts)} facts → {display_path(OUT)} ({size_kb:.1f} KB)")
    return 0
