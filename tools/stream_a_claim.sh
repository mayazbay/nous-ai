...
#
# Stale criteria (s74 fix — was over-eager):
#   - acquired_epoch + 7200s (2h) < now → stale
#
# We do NOT gate on `kill -0 PID` because the stored PID is the acquire-script's
# own short-lived `$$` (exits immediately after writing the lock), which would
# mark every freshly-acquired lock stale on the very next status call. Time-only
# staleness (2h cap) is the correct bound; if a session crashes silently the
# lock self-clears within 2h. PID is retained in the lock JSON for diagnostics.
#
...
is_active() {
  # $1 = pid (unused — kept for backward-compat callers), $2 = acquired_epoch
  # Time-only check (s74 fix): PID was the script's own $$, dead after exit.
  local epoch="$2"
  [ -n "$epoch" ] && [ "$epoch" -gt 0 ] 2>/dev/null && [ "$((NOW_EPOCH - epoch))" -le "$MAX_AGE" ] 2>/dev/null
}
