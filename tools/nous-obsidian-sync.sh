...
resolve_nous_vault() {
    # The same tracked wrapper is deployed on Mac and Air. Prefer Air's
    # always-on runtime clone when present; fall back to the Mac Documents vault.
    for candidate in \
        "${NOUS_VAULT_OVERRIDE:-}" \
        "/Users/madia/nous-agaas/wiki" \
...
if ! NOUS_VAULT="$(resolve_nous_vault)"; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] FAIL: no readable Nous git vault found; set NOUS_VAULT_OVERRIDE" >> "$LOG"
    exit 0
fi
...
acquire_lock() {
    LOCK_DIR="${TMPDIR:-/tmp}/nous-obsidian-sync.lock"
    ...
    if [ -n "$lock_pid" ] && kill -0 "$lock_pid" 2>/dev/null; then
        log_line "skip: another obsidian-sync run active (pid=$lock_pid)"
        return 1
    fi

    # Stale lock: remove only the pid file + empty lock directory, then retry once.
    ...
    log_line "skip: obsidian-sync lock busy"
    return 1
}

canonical_remote() {
    local vault="$1"
    if git -C "$vault" remote 2>>"$LOG" | grep -qx 'vps'; then
        printf 'vps\n'
        return 0
    fi
    if git -C "$vault" remote 2>>"$LOG" | grep -qx 'origin'; then
        printf 'origin\n'
        return 0
    fi
    return 1
...
sync_vault() {
    ...
    # Canonical VPS-bare remote is named `vps` on Mac and `origin` on Air.
    local CANONICAL_REMOTE
    if ! CANONICAL_REMOTE="$(canonical_remote "$vault")"; then
        log_line "$label skip: no vps/origin remote configured"
        return 0
    fi
    ...
    # 2. Fetch VPS state
    if ! git -C "$vault" fetch "$CANONICAL_REMOTE" main -q 2>>"$LOG"; then
        log_line "$label FAIL: fetch $CANONICAL_REMOTE main returned non-zero"
        return 1
    fi

    # 3. Try rebase canonical/main. If it succeeds, great. If it fails with a conflict,
    ...
    if ! git -C "$vault" rebase "$CANONICAL_REMOTE/main" -q 2>>"$LOG"; then
        git -C "$vault" rebase --abort 2>>"$LOG"
        if ! git -C "$vault" merge -X theirs "$CANONICAL_REMOTE/main" -q \
    ...
    if git -C "$vault" push "$CANONICAL_REMOTE" main -q 2>>"$LOG"; then
        local head_line
        head_line=$(git -C "$vault" log --oneline -1 2>>"$LOG")
        log_line "$label OK: $head_line via $CANONICAL_REMOTE"
    else
        log_line "$label FAIL: push $CANONICAL_REMOTE main returned non-zero"
        return 1
    fi
    ...
    if git -C "$vault" remote get-url github >/dev/null 2>&1; then
        ...
        if ! git -C "$vault" push "$CANONICAL_REMOTE" main -q 2>>"$LOG"; then
            log_line "$label FAIL: push $CANONICAL_REMOTE main after github rebase returned non-zero"
            return 1
        fi
...
# Log start of run
log_line "run started (vault=$NOUS_VAULT)"
acquire_lock || exit 0
