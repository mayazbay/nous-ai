...
PROOF_CHANNEL_PATTERNS = {
    "notion": re.compile(r"(?i)https://(?:www\.)?notion\.so/"),
    "google_drive": re.compile(r"(?i)https://(?:drive|docs)\.google\.com/"),
    "github": re.compile(r"(?i)https://github\.com/"),
    "vault": re.compile(r"(?i)(pages/(projects|systems|audits|proof-pack|task-results|progress|skills)/|\[\[[^\]]+\]\])"),
}
SOURCE_CHANNEL_PATTERNS = {
    "gbrain": re.compile(r"(?i)\bgbrain\b"),
    "openbrain": re.compile(r"(?i)\bopenbrain\b|\bopen brain\b"),
    "qmd": re.compile(r"(?i)\bqmd\b"),
    "source_finder": re.compile(r"(?i)source-finder|источник:\s*source"),
    "notion": PROOF_CHANNEL_PATTERNS["notion"],
    "google_drive": PROOF_CHANNEL_PATTERNS["google_drive"],
    "github": PROOF_CHANNEL_PATTERNS["github"],
    "vault": PROOF_CHANNEL_PATTERNS["vault"],
}
EVIDENCE_CHANNEL_LABELS = {
    "notion": "Notion",
...
def evidence_summary(item: dict[str, Any], notes: list[dict[str, Any]]) -> dict[str, Any]:
    """Summarize real evidence channels without treating Todoist itself as proof."""

    texts = [str(item.get("content") or ""), str(item.get("description") or "")]
    texts.extend(str(note.get("content") or "") for note in notes)
    combined = "\n".join(texts)
    proof_channels = sorted(name for name, pattern in PROOF_CHANNEL_PATTERNS.items() if pattern.search(combined))
    if any(note.get("file_attachment") for note in notes):
        proof_channels.append("attachment")
    source_channels = sorted(name for name, pattern in SOURCE_CHANNEL_PATTERNS.items() if pattern.search(combined))
    if "attachment" in proof_channels and "attachment" not in source_channels:
        source_channels.append("attachment")
    return {
        "source_backed": bool(source_channels or proof_channels),
        "human_checkable": bool(proof_channels),
        "close_ready": "notion" in proof_channels and "google_drive" in proof_channels,
...
def _channel_list(names: list[str]) -> str:
    labels = [EVIDENCE_CHANNEL_LABELS.get(name, name) for name in names]
    return ", ".join(labels) if labels else "нет"


def evidence_digest_line(row: dict[str, Any]) -> str:
    evidence = row.get("evidence") or {}
    if evidence.get("close_ready"):
        return (
            "Проверка фабрики: есть Notion + Google Drive proof; "
            "можно проверять закрытие после человеческой сверки."
        )
    if evidence.get("human_checkable"):
        return (
            "Проверка фабрики: доказательство есть "
            f"({_channel_list(list(evidence.get('proof_channels') or []))}), "
            "но правило закрытия не пройдено: нужны Notion + Google Drive вместе."
        )
    if evidence.get("source_backed"):
        return (
            "Проверка фабрики: источник есть "
            f"({_channel_list(list(evidence.get('source_channels') or []))}), "
            "но проверяемое доказательство не найдено."
        )
    return "Проверка фабрики: доказательство/источник не найден в задаче или комментариях; нужен source-finder."


def next_step_for_digest(row: dict[str, Any]) -> str:
    evidence = row.get("evidence") or {}
    if evidence.get("close_ready"):
        return "проверить доказательство и закрыть задачу или написать остаточный блокер."
    if not evidence.get("source_backed"):
        return "приложить реальный источник/доказательство или запустить source-finder; потом обновить статус."
    if row.get("status") == "blocked":
        return "написать точный блокер: что мешает, кто снимает, срок, какое доказательство нужно."
    return "обновить фактический статус и приложить недостающее доказательство в Notion/Drive/Obsidian."
...
def build_plan(
    ...
    *,
    ...
    for item in sync.get("items", []):
        ...
        considered += 1
        task_id = str(item.get("id") or "")
        task_notes = grouped_notes.get(task_id, [])
        ...
        selected[owner].append(
            {
                ...
                "url": task_url(item),
                "labels": labels,
                "evidence": evidence_summary(item, task_notes),
            }
        )
...
def digest_for_owner(owner: str, rows: list[dict[str, Any]]) -> str:
    ...
    for row in rows:
        ...
        lines.append(f"  {evidence_digest_line(row)}")
        lines.append(f"  Следующий шаг: {next_step_for_digest(row)}")
        lines.append(f"  Todoist: {row['url']}")
    lines.append("")
    lines.append(
        "Внутренние AI-комментарии в чат не выводятся. Чат видит статус, блокер, проверку фабрики, следующий шаг и ссылку Todoist."
    )
    return "\n".join(lines)
...
def apply_reminders(
    ...
    *,
    ...
    telegram_rows: dict[str, list[dict[str, Any]]] = collections.defaultdict(list)
        ...
        for owner, rows in plan["reminders"].items():
            for row in rows:
                ...
                if no_todoist_comments or not todoist_reminder_comments:
                    ...
                    telegram_rows[owner].append(row)
                    continue
                if dry_run:
    ...
    if not no_telegram and not comment_sweep_only:
        ...
        for owner, rows in telegram_rows.items():
            if not rows:
                continue
