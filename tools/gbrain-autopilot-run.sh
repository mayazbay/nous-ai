...
# Cron: */5 * * * * GBRAIN_AUTOPILOT_ONCE=1 '/root/.gbrain/autopilot-run.sh' >> '/root/.gbrain/autopilot.log' 2>&1
...
run_cycle() {
  local start elapsed cycle_tmp embed_tmp
  local sub_rc tee_rc embed_rc embed_tee1_rc embed_tee2_rc link_rc link_tee_rc
  ...
  phase_status=("${PIPESTATUS[@]}")
  embed_rc="${phase_status[0]:-1}"
  embed_tee1_rc="${phase_status[1]:-1}"
  embed_tee2_rc="${phase_status[2]:-1}"
  set -e
  ...
  phase_status=("${PIPESTATUS[@]}")
  link_rc="${phase_status[0]:-1}"
  link_tee_rc="${phase_status[1]:-1}"
  set -e
