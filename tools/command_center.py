...
# AP-44 dynamic subscription rotation (moonlit-pnueli Stage 6 doctrine,
# ceo-hierarchy v1.11.0). Import is best-effort: if the module is missing the
# feature flag below will hard-skip and existing grok-ceo fallback is unchanged.
try:
    from subscription_rotation import rotate_codex_to_fallback as _ap44_rotate
except ImportError:
    _ap44_rotate = None
try:
...
# AP-44 dynamic subscription rotation feature flag (default OFF — explicit
# opt-in until the rotation chain proves stable for ≥48h per
# COUNCIL-2026-05-23-plan-review action item #5).
NOUS_AP44_ROTATION_ENABLED = (
    os.environ.get("NOUS_AP44_ROTATION_ENABLED", "").strip().lower()
    in {"1", "true", "yes", "on"}
)
AP44_LEDGER_PATH = os.path.expanduser(
    os.environ.get("AP44_LEDGER_PATH",
                   "~/nous-agaas/wiki/pages/systems/ap44-rotation-ledger.jsonl")
)
# Footer marker shown to user when AP-44 rotation handles their query.
# Set NOUS_AP44_HIDE_TIER_FOOTER=1 to suppress.
AP44_HIDE_TIER_FOOTER = (
    os.environ.get("NOUS_AP44_HIDE_TIER_FOOTER", "").strip().lower()
    in {"1", "true", "yes", "on"}
)
...
# OpenClaw probe sentinel regex — workers reply with this literal string when a
# health probe asks "Reply exactly OPENCLAW_<N>_WORKER_OK". It MUST never reach
# a Telegram user (Assylbek incident 2026-05-20 18:57/18:58 KZT). See
# pages/skills/openclaw-probe-isolation/SKILL.md AP-1, AP-4.
OPENCLAW_WORKER_OK_RE = re.compile(r"^\s*OPENCLAW_\d+_WORKER_OK\s*$")
OPENCLAW_SENTINEL_FALLBACK_REPLY = (
    "⚠️ Фабрика жива, но рабочий вернул только служебный сигнал "
    "(probe sentinel), а не ответ на этот вопрос. Это операционный пробник, "
    "не данные. Переформулируйте запрос или попробуйте ещё раз через "
    "несколько секунд."
)
...
def _failover_capture_enabled() -> bool:
    """Return whether command handlers may mutate the durable failover ledger."""
    flag = os.environ.get("NOUS_FAILOVER_CAPTURE")
    if flag is not None:
        return flag.strip().lower() not in {"0", "false", "no", "off"}
    # Pytest routes exercise production command handlers with mocked workers.
    # Without this default-off guard, a missed mock writes fake Telegram events
    # into the live model-failover ledger and can create a commit storm.
...
def _failover_start(command: str, msg_id: int, chat_id: int, query: str, *, model: str, via: str) -> str | None:
    if not _failover_capture_enabled():
        return None
...
def _tg_send(bot_token: str, chat_id: int, text: str, reply_to: int | None = None) -> bool:
    ...
    # AP-4 (openclaw-probe-isolation): final defense — a sentinel must never
    # leave _tg_send. Even if some new caller bypasses the _run_openclaw
    # guard, this catches it at the boundary.
    if OPENCLAW_WORKER_OK_RE.match((text or "").strip()):
        log.error(
            "_tg_send BLOCKED probe sentinel: chat=%s text=%r",
            chat_id, (text or "")[:80],
        )
        text = OPENCLAW_SENTINEL_FALLBACK_REPLY
...
# Grok/xAI Premium image+video Telegram commands (P1.4 moonlit-pnueli Stage 10).
# Routes /grok-image, /grok-video + Russian aliases to tools/grok_image_gen.py
# and tools/grok_video_gen.py. xAI Imagine API returns hosted URLs that Telegram
# fetches via sendPhoto/sendVideo; no multipart upload needed.
GROK_TOOL_COMMANDS = {
    "/grok-image":     ("tools/grok_image_gen.py", "image"),
    "/грок-картинка":  ("tools/grok_image_gen.py", "image"),
    "/grok-video":     ("tools/grok_video_gen.py", "video"),
    "/грок-видео":     ("tools/grok_video_gen.py", "video"),
}


def _tg_send_photo(bot_token: str, chat_id: int, photo_url: str,
                   reply_to: int | None = None, caption: str = "") -> bool:
    """Send a photo by URL via Telegram sendPhoto. Telegram fetches the URL itself.

    Returns True on success. Falls back to plain text send with the URL if
    sendPhoto rejects (e.g. URL too long or content type mismatch).
    """
    params: dict = {"chat_id": str(chat_id), "photo": photo_url}
    if reply_to:
        params["reply_to_message_id"] = str(reply_to)
    if caption:
        params["caption"] = caption[:1024]  # Telegram caption max
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as resp:
            result = json.loads(resp.read())
            if result.get("ok"):
                log.info("sendPhoto OK: chat=%s msg_id=%s", chat_id, result.get("result", {}).get("message_id"))
                return True
            log.warning("sendPhoto rejected: %s — falling back to text URL", result)
    except Exception as exc:
        log.warning("sendPhoto raised: %s: %s — falling back to text URL", type(exc).__name__, exc)
    # Fallback: send the URL as plain text so Madi still gets the artifact
    fallback_text = f"✅ Image: {photo_url}\n{caption}" if caption else f"✅ Image: {photo_url}"
    return _tg_send(bot_token, chat_id, fallback_text, reply_to=reply_to)


def _tg_send_video(bot_token: str, chat_id: int, video_url: str,
                   reply_to: int | None = None, caption: str = "") -> bool:
    """Send a video by URL via Telegram sendVideo. Falls back to text URL on error."""
    params: dict = {"chat_id": str(chat_id), "video": video_url, "supports_streaming": "true"}
    if reply_to:
        params["reply_to_message_id"] = str(reply_to)
    if caption:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as resp:
            result = json.loads(resp.read())
            if result.get("ok"):
                log.info("sendVideo OK: chat=%s msg_id=%s", chat_id, result.get("result", {}).get("message_id"))
                return True
            log.warning("sendVideo rejected: %s — falling back to text URL", result)
    except Exception as exc:
        log.warning("sendVideo raised: %s: %s — falling back to text URL", type(exc).__name__, exc)
    fallback_text = f"✅ Video: {video_url}\n{caption}" if caption else f"✅ Video: {video_url}"
    return _tg_send(bot_token, chat_id, fallback_text, reply_to=reply_to)


def _handle_grok_tool(bot_token: str, chat_id: int, msg_id: int,
                      command: str, prompt: str) -> bool:
    """Run grok_image_gen.py / grok_video_gen.py and post the artifact to Telegram.

    Returns True if the message was handled (even on adapter failure — we still
    posted a user-visible error).
    """
    tool_path, kind = GROK_TOOL_COMMANDS[command]
    if not prompt.strip():
        _tg_send(
            bot_token, chat_id,
            f"Usage: <code>{command} &lt;prompt&gt;</code>\nExample: <code>{command} A neon city skyline at dusk</code>",
            reply_to=msg_id,
        )
        return True
    _tg_progress(
        bot_token, chat_id,
        f"⏳ Generating {kind} via xAI Imagine API…\n<code>{prompt[:120]}</code>\n"
        f"<i>{'Video takes up to 10 min (async polling). I will reply when ready.' if kind == 'video' else 'Image typically ~10s.'}</i>",
        msg_id,
    )
    env = os.environ.copy()
    env.setdefault("NOUS_PAID_API_ALLOWED", "1")
    env.setdefault("NOUS_PAID_API_CAP_USD", "5.00")
    env.setdefault("NOUS_PAID_API_REASON",
                   f"telegram {command} from chat={chat_id} msg={msg_id}")
    env.setdefault("NOUS_WIKI", "/Users/madia/nous-agaas/wiki")
    args = ["python3", tool_path, "--prompt", prompt, "--json"]
    if kind == "image":
        args.extend(["--response-format", "url"])
    timeout = 900 if kind == "video" else 180
    try:
        proc = subprocess.run(
            args, capture_output=True, text=True, env=env, timeout=timeout,
            cwd="/Users/madia/nous-agaas/wiki",
        )
    except subprocess.TimeoutExpired:
        _tg_send(bot_token, chat_id,
                 f"🔴 <code>{command}</code> timeout after {timeout}s. The xAI API may be slow; try again or shorten the prompt.",
                 reply_to=msg_id)
        log.warning("%s timeout chat=%s prompt=%r", command, chat_id, prompt[:80])
        return True
    if proc.returncode != 0:
        err = (proc.stderr or proc.stdout)[:400]
        _tg_send(bot_token, chat_id,
                 f"🔴 <code>{command}</code> rc={proc.returncode}:\n<pre>{_html.escape(err)}</pre>",
                 reply_to=msg_id)
        log.warning("%s rc=%s chat=%s err=%r", command, proc.returncode, chat_id, err[:200])
        return True
    try:
        result = json.loads(proc.stdout.strip().splitlines()[-1])
    except (json.JSONDecodeError, IndexError):
        _tg_send(bot_token, chat_id,
                 f"🔴 <code>{command}</code>: non-JSON output:\n<pre>{_html.escape(proc.stdout[:400])}</pre>",
                 reply_to=msg_id)
        log.warning("%s non-JSON output chat=%s", command, chat_id)
        return True
    if not result.get("ok"):
        err = result.get("error", "unknown error")
        _tg_send(bot_token, chat_id, f"🔴 <code>{command}</code>: {_html.escape(str(err))[:400]}",
                 reply_to=msg_id)
        log.warning("%s adapter not-ok chat=%s err=%r", command, chat_id, err)
        return True
    # Success path — post the artifact
    cost = result.get("cost_usd_est", 0.0) or result.get("cost_usd", 0.0) or 0.0
    latency = result.get("latency_ms", 0)
    caption_html = f"<i>via grok-imagine-{kind}, ${cost:.4f}, {latency}ms</i>"
    if kind == "image":
        artifacts = result.get("artifacts", [])
        photo_url = None
        if artifacts:
            artifact_path = artifacts[0]
            full = os.path.join("/Users/madia/nous-agaas/wiki", artifact_path)
            if artifact_path.endswith(".url.txt") and os.path.exists(full):
                try:
                    photo_url = open(full, encoding="utf-8").read().strip()
                except OSError:
                    photo_url = None
        if photo_url:
            _tg_send_photo(bot_token, chat_id, photo_url, reply_to=msg_id, caption=caption_html)
        else:
            _tg_send(bot_token, chat_id,
                     f"✅ {command} ok but no image URL found.\nArtifacts: {artifacts}",
                     reply_to=msg_id)
    else:  # video
        video_url = result.get("video_url")
        if video_url:
            _tg_send_video(bot_token, chat_id, video_url, reply_to=msg_id, caption=caption_html)
        else:
            artifact = result.get("artifact")
            _tg_send(bot_token, chat_id,
                     f"✅ {command} ok but no video URL.\nArtifact: {artifact}",
                     reply_to=msg_id)
    log.info("%s OK chat=%s msg_id=%s kind=%s cost=$%.4f", command, chat_id, msg_id, kind, cost)
    return True
...
def _is_openclaw_probe_sentinel(text: str) -> bool:
    """True iff the response is a worker-health probe sentinel.

    The factory worker echoes `OPENCLAW_<N>_WORKER_OK` only when an operator
    probe explicitly asks it to. That string MUST never reach a Telegram user.
    See pages/skills/openclaw-probe-isolation/SKILL.md AP-1, AP-4.
    """
    if not text:
        return False
    return bool(OPENCLAW_WORKER_OK_RE.match(text))


def _log_probe_sentinel_leak(correlation_id: str, query: str, response: str) -> None:
    """Defense-in-depth: any sentinel that hit the boundary is a routing bug.

    Logs to stderr + best-effort append to mistake-to-skill ledger.jsonl.
    """
    log.error(
        "probe_sentinel_leaked DROPPED: correlation_id=%s q=%r r=%r",
        correlation_id, (query or "")[:80], (response or "")[:80],
    )
    try:
        from datetime import datetime as _dt
        ledger = Path("/Users/madia/nous-agaas/wiki/pages/skills/mistake-to-skill/ledger.jsonl")
        if ledger.parent.exists():
            entry = {
                "ts": _dt.now().isoformat(),
                "kind": "probe_sentinel_leaked",
                "correlation_id": correlation_id or "",
                "query_preview": (query or "")[:120],
                "response_preview": (response or "")[:120],
                "skill": "openclaw-probe-isolation",
                "ap": "AP-1+AP-4",
            }
...
def _run_openclaw(query: str, timeout: int = ASK_TIMEOUT, model: str = None,
    ...
    Sentinel guard (AP-1, AP-4 openclaw-probe-isolation, 2026-05-20 Assylbek
    incident): if the factory returns a `OPENCLAW_<N>_WORKER_OK` probe
    sentinel, it is DROPPED here and replaced with a human-readable message.
    A sentinel must never reach `_tg_send`.
    """
    try:
        ...
        if proc.returncode == 0:
            stdout = proc.stdout.strip() or "(agent returned no output)"
            if _is_openclaw_probe_sentinel(stdout):
                _log_probe_sentinel_leak(correlation_id, query, stdout)
                return OPENCLAW_SENTINEL_FALLBACK_REPLY
            return stdout
        else:
...
def is_command(text: str) -> bool:
    ...
    return (
        ...
        or t.startswith("/last-plate")
        or t.startswith("/last-photo")
    )
...
# ---------------------------------------------------------------------------
# AP-44 Dynamic Subscription Rotation helpers (ceo-hierarchy v1.11.0)
# Pattern: Codex blocked → try Claude/Opus subscription → try OpenAI API →
# only THEN fall to grok-ceo. Feature-flagged so prod behavior is unchanged
# until NOUS_AP44_ROTATION_ENABLED=1 is explicitly set in the launchd env.
# ---------------------------------------------------------------------------

def _ap44_log_rotation(event: dict) -> None:
    """Best-effort append to pages/systems/ap44-rotation-ledger.jsonl.

    Never raises, never blocks the caller. Surfaces tier transitions for the
    Sat-03:00 council digest + post-mortem analysis per Opus's Stage 7 council
    observability gap (COUNCIL-2026-05-23-plan-review action item #1).
    """
    try:
        os.makedirs(os.path.dirname(AP44_LEDGER_PATH), exist_ok=True)
        with open(AP44_LEDGER_PATH, "a", encoding="utf-8") as f:
            f.write(json.dumps(event, ensure_ascii=False) + "\n")
    except Exception as exc:  # noqa: BLE001 — ledger MUST NOT block caller
        log.warning("AP-44 ledger write failed: %s: %s", type(exc).__name__, exc)


def _try_ap44_rotation(query: str, chat_id, msg_id, today_spend: float,
                       route: str) -> tuple[bool, str | None]:
    """Try Claude subscription → OpenAI API. Return (handled, answer_text).

    handled=True → caller posts answer_text and returns; do NOT fall to grok-ceo.
    handled=False → rotation skipped (flag off / module missing) OR rotation failed;
                     caller proceeds with the existing grok-ceo fallback path.

    Idempotent ledger row written for every invocation (including skips when the
    flag is off — so we can audit how often we WOULD have rotated had it been on).
    """
    ...
    base_event = {
        ...
        "msg_id": msg_id,
        "route": route,
        "today_codex_spend_usd": round(float(today_spend or 0.0), 4),
        "flag_enabled": NOUS_AP44_ROTATION_ENABLED,
        "module_present": _ap44_rotate is not None,
    }

    if not NOUS_AP44_ROTATION_ENABLED or _ap44_rotate is None:
        base_event["tier"] = "skipped"
        base_event["ok"] = False
        base_event["reason"] = (
            "flag_off" if not NOUS_AP44_ROTATION_ENABLED else "module_missing"
        )
        _ap44_log_rotation(base_event)
        return (False, None)

    # Set NOUS_PAID_API_REASON so the OpenAI fallback's paid-API gate accepts.
    # Do NOT override if caller already set it.
    os.environ.setdefault(
        "NOUS_PAID_API_REASON",
        f"ap44 rotation; codex blocked; chat={chat_id} msg={msg_id} "
        f"route={route} today_spend=${today_spend:.2f}",
    )

    try:
        result = _ap44_rotate(query, skip_openai=False)
    except Exception as exc:  # noqa: BLE001 — rotation MUST NOT crash caller
        base_event.update({
            "tier": "exception",
            "ok": False,
        ...
        _ap44_log_rotation(base_event)
        log.warning("AP-44 rotation raised: %s: %s; falling back to grok-ceo",
    ...
    if result.get("ok"):
        ...
        if AP44_HIDE_TIER_FOOTER:
            return (True, answer)
        footer = f"\n\n_via {tier}, ${cost:.4f} (AP-44 rotation)_"
...
def handle(bot_token: str, chat_id: int, msg_id: int, text: str) -> bool:
    ...
    # /grok-image, /grok-video + Russian aliases ─────────
    # P1.4 (moonlit-pnueli Stage 10): xAI Imagine API image+video via Telegram.
    # Match BEFORE /handoff because the prefixes are distinct and we want
    # adapter cost-gates to fire on the worker subprocess, not the main bot.
    for _gcmd in GROK_TOOL_COMMANDS:
        if lower.startswith(_gcmd):
            _prompt = t[len(_gcmd):].strip()
            return _handle_grok_tool(bot_token, chat_id, msg_id, _gcmd, _prompt)
    ...
    # /last-plate, /last-photo ────────────────────────────
    # Read-side query for the last recognized Hikvision camera event.
    # Doctrine: pages/skills/camera-event-query/SKILL.md.
    if lower.startswith("/last-plate"):
        try:
            from tools import satory_camera_query
        except ImportError:
            import satory_camera_query
        try:
            reply = satory_camera_query.get_last_plate_reply()
        except Exception as exc:  # noqa: BLE001 - never crash the bot on this
            log.exception("/last-plate failed: %s", exc)
            reply = "📷 Не удалось получить последний номер (внутренняя ошибка). Попробуй ещё раз через минуту."
        _tg_send(bot_token, chat_id, reply, reply_to=msg_id)
        log.info(f"/last-plate handled: chat={chat_id} r_len={len(reply)}")
        return True

    if lower.startswith("/last-photo"):
        try:
            from tools import satory_camera_query
        except ImportError:
            import satory_camera_query
        try:
            reply = satory_camera_query.get_last_photo_reply()
        except Exception as exc:  # noqa: BLE001
            log.exception("/last-photo failed: %s", exc)
            reply = "📷 Не удалось получить последнее фото (внутренняя ошибка). Попробуй ещё раз через минуту."
        _tg_send(bot_token, chat_id, reply, reply_to=msg_id)
        log.info(f"/last-photo handled: chat={chat_id} r_len={len(reply)}")
        return True
    ...
    if lower.startswith("/ask"):
        ...
        if _requires_codex_verification_route(query):
            ...
            if not _budget_ok:
                ...
                # AP-44: try Claude subscription → OpenAI API BEFORE grok-ceo.
                # Flag-gated; when off, falls through to existing grok-ceo path.
                _ap44_ok, _ap44_answer = _try_ap44_rotation(
                    query, chat_id, msg_id, _today_spend,
                    route="shell-verification-budget-closed",
                )
                if _ap44_ok:
                    response = _operator_visible_response(chat_id, _ap44_answer)
                    receipt = _write_telegram_task_result_receipt(
                        "/ask-ap44-rotation", msg_id, query, response,
                        model="ap44-rotation",
                        via="/ask shell-verification AP-44 rotation (codex blocked)",
                    )
                    if len(response) > MAX_MSG_LEN:
                        suffix = f"\n\n…(truncated)\n📄 Full: wiki/{receipt}" if receipt else "\n\n… (truncated — receipt write failed)"
                        response = response[: MAX_MSG_LEN - len(suffix)] + suffix
                    ...
                    _tg_send(bot_token, chat_id, response, reply_to=msg_id)
                    _observe_telegram_command(
                        "/ask-ap44-rotation", msg_id,
                        query=query, response=response, status="ok",
                    )
                    return True
            ...
            if _is_codex_cap_blocked(response):
                ...
                # AP-44: try Claude → OpenAI BEFORE grok-ceo on mid-call cap.
                _ap44_ok2, _ap44_answer2 = _try_ap44_rotation(
                    query, chat_id, msg_id, _today_spend,
                    route="shell-verification-cap-mid-call",
                )
                if _ap44_ok2:
                    response = _ap44_answer2
                else:
                    fallback_query = query
                    if _is_group_chat(chat_id):
                        fallback_query = (
        ...
        if route == ROUTE_CHATGPT_EXECUTION:
            ...
            if _budget_ok:
                ...
                if _is_codex_cap_blocked(response):
                    ...
                    # AP-44: try Claude → OpenAI BEFORE grok-ceo on langgraph-codex cap-mid-call.
                    _ap44_ok4, _ap44_answer4 = _try_ap44_rotation(
                        query, chat_id, msg_id, _today_spend,
                        route="langgraph-codex-execution-cap-mid-call",
                    )
                    if _ap44_ok4:
                        response = _ap44_answer4
                    else:
                        fallback_query = query
                        if _is_group_chat(chat_id):
                            fallback_query = (
            ...
            if _is_mandatory_codex_decision(route_decision):
                ...
                is_group = _is_group_chat(chat_id)
                # AP-44: try Claude → OpenAI BEFORE grok-ceo on mandatory-codex blocked.
                # Flag-gated; when off, falls through to existing grok-ceo notice path.
                _ap44_ok3, _ap44_answer3 = _try_ap44_rotation(
                    query, chat_id, msg_id, _today_spend,
                    route="mandatory-codex-blocked",
                )
                if _ap44_ok3:
                    response = _operator_visible_response(chat_id, _ap44_answer3)
                    receipt = _write_telegram_task_result_receipt(
                        "/ask-ap44-rotation", msg_id, query, response,
                        model="ap44-rotation",
                        via=f"/ask mandatory-codex AP-44 rotation ({'group' if is_group else 'dm'})",
                    )
                    if len(response) > MAX_MSG_LEN:
                        suffix = f"\n\n…(truncated)\n📄 Full: wiki/{receipt}" if receipt else "\n\n… (truncated — receipt write failed)"
                        response = response[: MAX_MSG_LEN - len(suffix)] + suffix
                    ...
                    _observe_telegram_command(
                        "/ask-ap44-rotation", msg_id,
                        query=query, response=response, status="ok",
                        metadata={"route": route, "ap44_route": "mandatory-codex-blocked",
        ...
        if _query_likely_needs_high_judgment(query):
            ...
            if _is_codex_cap_blocked(response):
                ...
                # AP-44: try Claude → OpenAI BEFORE grok-ceo on high-judgment cap-mid-call.
                _ap44_ok5, _ap44_answer5 = _try_ap44_rotation(
                    query, chat_id, msg_id, _today_spend,
                    route="high-judgment-cap-mid-call",
                )
                if _ap44_ok5:
                    response = _ap44_answer5
                else:
                    response = _run_openclaw(query, agent_id="grok-ceo", correlation_id=f"tg_{msg_id}")
            response = _operator_visible_response(chat_id, response)
            receipt = _write_telegram_task_result_receipt(
