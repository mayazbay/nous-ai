...
def snapshot(wiki: Path, fixture_mode: bool = False, dry_run: bool = False) -> PhaseResult:
    ...
    if dry_run:
        log(f"  dry_run — would write snapshot → {snap_path}")
        return PhaseResult(phase="snapshot", status="ok", detail=f"dry_run would write {snap_path}", data=data)
...
def digest(
    ...
    if dry_run:
        publish_result = {
            "karpathy_table": "dry_run",
            "gbrain_push": "dry_run",
            "hermes_reseed": "dry_run",
        }
        log(f"  dry_run — would write digest → {audit_path}")
        log(f"  publish surfaces: {publish_result}")
        return PhaseResult(
            phase="digest",
            status="ok",
            detail=f"dry_run would write {audit_path}",
            data={"publish": publish_result, "body": body},
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        # Phase 1
        p1 = snapshot(wiki, fixture_mode=args.fixture_mode, dry_run=args.dry_run)
        phases.append(p1)
    ...
    # Write state. In dry-run mode, leave even the state file untouched so
    # read-only audits cannot manufacture a fresh "last_run_at" proof.
    if args.dry_run:
        log(f"dry_run — state not written → {state_path}")
    else:
