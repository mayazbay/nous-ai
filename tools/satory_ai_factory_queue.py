...
def queue_diagnostics(report: dict[str, Any], ledger: dict[str, Any], *, priority: str) -> dict[str, Any]:
    """Explain why a dry-run selected zero tasks without mutating the ledger."""

    queued_candidates = 0
    delete_candidate_skipped = 0
    ledger_blocked_candidates = 0
    unblocked: list[dict[str, Any]] = []
    ledger_blocked_samples: list[dict[str, Any]] = []
    unblocked_samples: list[dict[str, Any]] = []
    for row in report.get("tasks", []):
        if row.get("execution_state") != "queued":
            continue
        queued_candidates += 1
        if row.get("delete_candidate_reason"):
            delete_candidate_skipped += 1
            continue
        event_id = event_fingerprint(row)
        blocked_reason = ""
        if already_ran(ledger, event_id):
            blocked_reason = "event_already_ran"
        elif task_already_ran_without_new_signal(ledger, row):
            blocked_reason = "task_already_ran_without_new_signal"
        if blocked_reason:
            ledger_blocked_candidates += 1
            if len(ledger_blocked_samples) < 5:
                ledger_blocked_samples.append(
                    {
                        "task_id": row.get("task_id"),
                        "title": row.get("content"),
                        "event_id": event_id,
            ...
            continue
        unblocked.append({"event_id": event_id, "task": row})

    unblocked.sort(key=lambda event: priority_score(event["task"], priority))
    for event in unblocked[:5]:
        row = event["task"]
    ...
    return {
        ...
        "delete_candidate_skipped": delete_candidate_skipped,
        "eligible_candidates": queued_candidates - delete_candidate_skipped,
        "ledger_blocked_candidates": ledger_blocked_candidates,
        "unblocked_candidates": len(unblocked),
        "ledger_blocked_samples": ledger_blocked_samples,
...
def main(argv: list[str] | None = None) -> int:
    ...
    diagnostics = queue_diagnostics(report, ledger, priority=args.priority)
