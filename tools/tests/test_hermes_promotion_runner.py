...
def test_main_refuses_non_air_without_writing_audit(tmp_path: Path, monkeypatch, capsys) -> None:
    monkeypatch.setattr(runner, "host_is_air_runtime", lambda: False)

    rc = runner.main(["--wiki", str(tmp_path), "--json"])

    assert rc == 2
    assert "must run on the Air production runtime" in capsys.readouterr().out
    assert not (tmp_path / "pages" / "audits").exists()


def test_main_allows_explicit_non_air_escape_hatch(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(runner, "host_is_air_runtime", lambda: False)

    rc = runner.main(["--wiki", str(tmp_path), "--allow-non-air", "--mock-all-green", "--json"])

    assert rc == 0
    assert (tmp_path / "pages" / "audits" / f"HERMES-PROMOTION-RUNNER-DEPLOY-PROOF-{dt.date.today().isoformat()}.md").exists()
