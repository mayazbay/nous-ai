...
# ---------------------------------------------------------------------------
# 12b. update_field — same value is idempotent
# ---------------------------------------------------------------------------

def test_update_field_same_value_noops(tmp_path: Path):
    wiki = tmp_path
    _write_file(wiki / "z.md", "# Z\n")
    uid = reg.add("z.md", wiki=wiki)
    assert reg.update_field(uid, "embed_model", "", wiki=wiki) is True
    assert reg.update_field(uid, "embed_dim", 0, wiki=wiki) is True
    assert len(_read_rows(wiki)) == 1

    assert reg.update_field(uid, "title", "Z", wiki=wiki) is True
    assert len(_read_rows(wiki)) == 1
