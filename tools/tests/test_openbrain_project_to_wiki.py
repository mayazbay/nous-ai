...
class OpenBrainProjectionTests(unittest.TestCase):
    ...
    def test_projection_is_idempotent_and_uses_full_uuid_filename(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('title: "KEONA / SPECTRA latest email correction, 2026-05-11"', rendered)
            self.assertIn("# KEONA / SPECTRA latest email correction, 2026-05-11", rendered)
            self.assertNotIn("latest email correction, 2026-05-11: Lim email from 2026-05-07 is verified.", rendered.splitlines()[3])

    def test_title_prefers_clear_sentence_before_long_parenthetical(self):
        title = openbrain.title_from_content(
            "2026-05-20 — Karpathy Council 8/8 closed "
            "(gbrain-ops v1.80.5, commit 6f76aa16). Follow-up details."
        )
        self.assertEqual(title, "2026-05-20 — Karpathy Council 8/8 closed")
    ...
    def test_mcp_call_forces_http11_for_sse_endpoint(self):
        payload = 'data: {"result":{"content":[{"type":"text","text":"No thoughts found"}]}}\n\n'

        def fake_run(cmd, **kwargs):
            self.assertIn("--http1.1", cmd)
            return subprocess.CompletedProcess(cmd, 0, stdout=payload, stderr="")

        with mock.patch.object(openbrain.subprocess, "run", side_effect=fake_run):
            text = openbrain.call_mcp_tool("https://openbrain.example/mcp", "list_thoughts", {"limit": 1})

        self.assertEqual(text, "No thoughts found")
