...
def test_gbrain_healthy_is_green() -> None:
    output = 'progress\n{"status":"healthy","health_score":100,"checks":[]}\n'

    status, detail, evidence = gate.status_from_gbrain_doctor(output, 0)

    assert status == "GREEN"
    assert "status=healthy" in detail
    assert evidence["status"] == "healthy"


def test_satory_queue_ledger_idle_is_green() -> None:
    status, detail = gate.status_from_satory_queue(
        {
            "selected": 0,
            "status": "ok",
            "diagnostics": {
                "queued_candidates": 27,
                "ledger_blocked_candidates": 27,
    ...
    assert status == "GREEN"
    assert "ledger-covered" in detail


def test_satory_queue_zero_without_diagnostics_is_yellow() -> None:
    status, detail = gate.status_from_satory_queue({"selected": 0, "status": "ok"})

    assert status == "YELLOW"
    assert "lacks queue diagnostics" in detail


def test_openclaw_current_runtime_green_when_legacy_canary_stopped(monkeypatch) -> None:
    monkeypatch.setattr(
        gate,
        "ssh",
        lambda *_args, **_kwargs: _cmd_result(
            ...
            stdout=(
                ...
                "inspect=ghcr.io/openclaw/openclaw:2026.5.19\trunning\thealthy\n"
                "legacy18790=curl: (7) Failed to connect to 127.0.0.1 port 18790\n"
            ),
        ),
    )

    result = gate.check_openclaw_current_runtime_health()

    assert result["status"] == "GREEN"
    assert "2026.5.19" in result["detail"]
...
def test_evaluate_does_not_send_phone_canary_by_default(monkeypatch, tmp_path: Path) -> None:
    ...
    def fake_ssh(host: str, script: str, **kwargs):
        ...
        if "control_plane_sync_loop.py" in script:
            return _cmd_result(stdout='{"overall_status":"done","steps":[]}')
        if "18789/health" in script:
            return _cmd_result(
                stdout='health={"ok":true,"status":"live"}\ninspect=ghcr.io/openclaw/openclaw:2026.5.19\trunning\thealthy\nlegacy18790=curl: (7)\n'
            )
        if "satory_ai_factory_queue.py" in script:
            return _cmd_result(stdout='{"selected":1,"results":[{"id":"t1"}],"diagnostics":{"unblocked_candidates":1}}')
