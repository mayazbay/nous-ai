...
RESTART_SCRIPT="${LOCAL_RESTART_CRITICAL_SCRIPT:-$(cd "$(dirname "$0")/.." && pwd)/tools/restart_critical.sh}"
...
check_runner_content() {
  ...
  grep -Fq 'phase_status=("${PIPESTATUS[@]}")' "$script" || {
    echo "$label autopilot script does not snapshot PIPESTATUS before assignment (AP-108)"
    fail=1
  }
  grep -Fq 'exit "$cycle_rc"' "$script" || {
    echo "$label autopilot script can return a false-green one-shot exit after run_cycle failure (AP-109)"
    fail=1
  }
...
check_local() {
  ...
  if [ ! -f "$RESTART_SCRIPT" ]; then
    echo "missing restart_critical script: $RESTART_SCRIPT"
    fail=1
  else
    bash -n "$RESTART_SCRIPT" || fail=1
    grep -Fq "GBRAIN_AUTOPILOT_ONCE=1" "$RESTART_SCRIPT" || {
      echo "restart_critical can recreate long-running gbrain autopilot without one-shot mode"
      fail=1
    }
    grep -Fq "scheduled_one_shot" "$RESTART_SCRIPT" || {
      echo "restart_critical does not treat the one-shot cron schedule as healthy"
      fail=1
    }
    if grep -Fq "nohup /root/.gbrain/autopilot-run.sh" "$RESTART_SCRIPT"; then
      echo "restart_critical still starts the gbrain autopilot daemon directly"
      fail=1
    fi
  fi
}
...
check_remote() {
  ...
  ssh -o ConnectTimeout=10 "$HOST" "grep -Fq 'test_no_lying_logs.py' '$REMOTE_SCRIPT' && grep -Fq 'tg_send.sh' '$REMOTE_SCRIPT' && grep -Fq 'cycle_tmp' '$REMOTE_SCRIPT' && grep -Fq 'AP-99' '$REMOTE_SCRIPT' && grep -Fq 'phase_status=(\"\${PIPESTATUS[@]}\")' '$REMOTE_SCRIPT' && grep -Fq 'exit \"\$cycle_rc\"' '$REMOTE_SCRIPT'" || {
    echo "remote autopilot script does not have AP-99/AP-108/AP-109 gate wiring (lying-log gate / PIPESTATUS snapshot / one-shot rc propagation)"
    fail=1
  }
  ...
  ssh -o ConnectTimeout=10 "$HOST" "crontab -l 2>/dev/null | grep -Fq \"GBRAIN_AUTOPILOT_ONCE=1 '/root/.gbrain/autopilot-run.sh'\"" || {
    echo "remote autopilot cron is not scheduled as a one-shot cycle"
    fail=1
  }
}
