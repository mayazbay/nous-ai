...
usage() {
  cat <<'EOF'
Usage:
  bash tools/factory_no_drift_probe.sh [--json|--quiet] [--no-telegram] [--no-repair]

Options:
  --json          Emit machine-readable JSON only.
  --quiet         Suppress progress text; still emits JSON.
  --no-telegram   Suppress drift alert delivery.
  --no-repair     Classify only; do not auto-pull/rebase.
  -h, --help      Print this help and exit without probing or writing.
EOF
}
...
for arg in "${@:-}"; do
  case "$arg" in
    ...
    -h|--help) usage; exit 0 ;;
    *) echo "factory_no_drift_probe.sh: unknown arg: $arg" >&2; usage >&2; exit 2 ;;
  esac
done
