...
def update_field(
    ...
    *,
    ...
    Idempotent: if the current value is already equal to ``value``, returns
    ``True`` without appending a duplicate row. This keeps generated queue
    drains from dirtying the git worktree with timestamp-only registry rows.
