...
def title_from_content(content: str, limit: int = 72) -> str:
    ...
    candidates: list[str] = []
    if ":" in one_line:
        candidates.append(one_line.split(":", 1)[0].strip())
    first_sentence = re.split(r"(?<=[.!?])\s+", one_line, 1)[0].strip()
    candidates.append(first_sentence)
    candidates.extend(re.sub(r"\s+\([^)]{8,}\)\.?$", "", candidate).strip() for candidate in list(candidates))
