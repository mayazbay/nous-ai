...
def _voyage_response(n: int, dim: int = 1024) -> _FakeResponse:
...
def test_voyage_embed_2xx_returns_vectors(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        lv.urllib.request, "urlopen", lambda *a, **kw: _voyage_response(2, dim=1024)
    )
    results = emb.embed(_chunks(2))
    assert len(results) == 2
    assert all(r.error == "" for r in results)
    assert all(len(r.vector) == 1024 for r in results)
    assert all(r.dim == 1024 for r in results)
...
def test_voyage_embed_batches_at_128(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def _fake(*_a, **_kw):
        ...
        return _voyage_response(len(body["input"]), dim=1024)
...
def test_voyage_embed_length_mismatch_returns_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        lv.urllib.request, "urlopen", lambda *a, **kw: _voyage_response(3, dim=1024)
    )
