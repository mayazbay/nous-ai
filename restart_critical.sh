...
check_autopilot() {
  ...
  if [ "$DRY_RUN" = "1" ]; then
    print_result "autopilot" "restart" "restarted" "dry_run_missing_process"
    return 0
  fi
  start_out="$(ssh -n -o BatchMode=yes -o ConnectTimeout=8 "$VPS_SSH" "nohup /root/.gbrain/autopilot-run.sh >> /root/.gbrain/autopilot.log 2>&1 & echo started_pid=\$!" 2>&1)"
